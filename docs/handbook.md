...
| `scripts/check-bundle-size.mjs` | size, CI                      | framework 180 KB, entry 60 KB, chunk lẻ 250 KB, CSS 30 KB, tổng JS 750 KB (gzip)                                                                                                                                                                                                 |
| CI label `guards`               | CI                            | PR sửa `eslint.config.js`, `vitest.config.ts`, `tsconfig.json`, `scripts/`, `.github/` mà không gắn label                                                                                                                                                                        |
...
- Build: `tsc --noEmit` rồi Vite 8 (rolldown). Framework (React, TanStack, Zustand, axios, i18next, zod, dayjs) tách một chunk để cache lâu; Ant Design tách theo route.
